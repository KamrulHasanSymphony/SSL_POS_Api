...
namespace ShampanPOS.Controllers
{
    ...
    public class UOMController : ControllerBase
    {
        ...
        // POST: api/UOM/Insert
        [HttpPost("Insert")]
        public async Task<ResultVM> Insert(UOMVM uom)
        {
            ...
            try
            {
                resultVM = await _uomService.Insert(uom);
                return resultVM;
            }
            catch (Exception ex)
            {
        ...
        // POST: api/UOM/Update
        [HttpPost("Update")]
        public async Task<ResultVM> Update(UOMVM uom)
        {
            ResultVM resultVM = new ResultVM { Status = "Fail", Message = "Error", ExMessage = null, Id = "0", DataVM = null };
            try
            {
                _uomService = new UOMService();
                resultVM = await _uomService.Update(uom);
                return resultVM;
            }
            catch (Exception ex)
            {
        ...
        public async Task<ResultVM> Delete(CommonVM vm)
        {
            ...
            try
            {
                ...
                resultVM = await _uomService.Delete(vm);
                return resultVM;
            }
            catch (Exception ex)
            {
        ...
        public async Task<ResultVM> List(CommonVM vm)
        {
            ...
            {
                 _uomService = new UOMService();
                resultVM = await _uomService.List(new[] { "H.Id" }, new[] { vm.Id}, null);
                return resultVM;
            }
            catch (Exception ex)
            {
        ...
        // GET: api/UOM/ListAsDataTable
        [HttpGet("ListAsDataTable")]
        public async Task<ResultVM> ListAsDataTable(UOMVM uom)
        {
            ResultVM resultVM = new ResultVM { Status = "Fail", Message = "Error", ExMessage = null, Id = "0", DataVM = null };
            try
            {
                _uomService = new UOMService();
                resultVM = await _uomService.ListAsDataTable(new[] { "" }, new[] { "" });
                return resultVM;
            }
            catch (Exception ex)
            {
        ...
        public async Task<ResultVM> Dropdown()
        {
            ...
            {
                UOMService _uomService = new UOMService();
                resultVM = await _uomService.Dropdown(); // Adjust if Dropdown requires a different method
                return resultVM;
            }
            catch (Exception ex)
            {
        ...
        public async Task<FileStreamResult> ReportPreview(CommonVM vm)
        {
            ...
            try
            {
                ...
                var resultVM = await _uomService.ReportPreview(new[] { "H.Id", "H.BranchId" }, new[] { vm.Id, vm.BranchId }, peramModel);
                ...
                if (resultVM.Status == "Success" && resultVM.DataVM is DataTable dt && dt.Rows.Count > 0)
                {
                    ...
                    var stream = httpRequestHelper.PostDataReport(baseUrl + "/api/UOM/GetUOM", authModel, json);
