    ...
    public class PurchaseOrderRepository : CommonRepository
    {
        ...
        public async Task<ResultVM> Insert(PurchaseOrderVM purchaseOrder, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                (Code, BranchId, SupplierId, BENumber, OrderDate, DeliveryDateTime, GrandTotalAmount, 
                GrandTotalSDAmount, GrandTotalVATAmount, Comments, TransactionType,
                CurrencyId, CurrencyRateFromBDT, FiscalYear, PeriodId,IsPost, CreatedBy,CreatedOn,CreatedFrom)
                VALUES 
                (@Code, @BranchId, @SupplierId, @BENumber, @OrderDate, @DeliveryDateTime, @GrandTotalAmount, 
                @GrandTotalSDAmount, @GrandTotalVATAmount, @Comments, @TransactionType, 
                @CurrencyId, @CurrencyRateFromBDT, @FiscalYear, @PeriodId,@IsPost, @CreatedBy,@CreatedOn,@CreatedFrom);
        ...
        public async Task<ResultVM> Update(PurchaseOrderVM purchaseOrder, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                SET 
                   SupplierId = @SupplierId, BENumber = @BENumber, OrderDate = @OrderDate, 
                    DeliveryDateTime = @DeliveryDateTime, GrandTotalAmount = @GrandTotalAmount, GrandTotalSDAmount = @GrandTotalSDAmount, 
                    GrandTotalVATAmount = @GrandTotalVATAmount, Comments = @Comments, TransactionType = @TransactionType, 
                     CurrencyId = @CurrencyId, CurrencyRateFromBDT = @CurrencyRateFromBDT,  FiscalYear = @FiscalYear, PeriodId = @PeriodId, 
...
WHERE  1 = 1
 ";
        ...
        public async Task<ResultVM> UpdateGrandTotal(PurchaseOrderVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            bool isNewConnection = false;
            ...
            try
            {
                ...
                UPDATE PurchaseOrders
                SET 
                    GrandTotalAmount = (SELECT SUM(SubTotal) FROM PurchaseOrderDetails WHERE PurchaseOrderId = @Id),
                    GrandTotalSDAmount = (SELECT SUM(SDAmount) FROM PurchaseOrderDetails WHERE PurchaseOrderId = @Id),
                    GrandTotalVATAmount = (SELECT SUM(VATAmount) FROM PurchaseOrderDetails WHERE PurchaseOrderId = @Id)
                WHERE Id = @Id ";
...
";
        ...
        public async Task<ResultVM> GetDetailsGridData(GridOptions options, string[] conditionalFields, string[] conditionalValues, SqlConnection conn, SqlTransaction transaction)
        {
            ...
            try
            {
                ...
                var data = new GridEntity<PurchaseOrderDetail>();
	...
	WHERE 1 = 1
    -- Add the filter condition
        " + (options.filter.Filters.Count > 0 ? " AND (" + GridQueryBuilder<PurchaseOrderDetail>.FilterCondition(options.filter) + ")" : "");
...
";
                ...
                data = KendoGrid<PurchaseOrderDetail>.GetTransactionalGridData_CMD(options, sqlQuery, "H.Id", conditionalFields, conditionalValues);
