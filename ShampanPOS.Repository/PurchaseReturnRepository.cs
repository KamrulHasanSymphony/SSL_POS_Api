...
namespace ShampanPOS.Repository
{
    ...
    public class PurchaseReturnRepository : CommonRepository
    {
         ...
         public async Task<ResultVM> Insert(PurchaseReturnVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                string query = @"
                    ...
                    (
                        Code, BranchId, SupplierId, BENumber, PurchaseReturnDate, 
                        GrandTotalAmount, GrandTotalSDAmount, GrandTotalVATAmount, Comments, 
                        TransactionType, IsPost, CurrencyId, CurrencyRateFromBDT, 
                        ImportIDExcel, FileName, FiscalYear, PeriodId, CreatedBy, CreatedOn,CreatedFrom
                    )
                    VALUES 
                    (
                        @Code, @BranchId, @SupplierId, @BENumber, @PurchaseReturnDate,
        ...
        (PurchaseReturnId, PurchaseId, PurchaseDetailId, BranchId, Line, ProductId,Quantity, UnitPrice, SubTotal, SD, SDAmount, VATRate, VATAmount,LineTotal,UOMId,UOMFromId,UOMConversion,IsPost,Comments,FixedVATAmount,IsFixedVAT,VATType,OthersAmount,TransactionType,ReturnReason,CtnQuantity,PcsQuantity)
        VALUES 
        (@PurchaseReturnId, @PurchaseId, @PurchaseDetailId, @BranchId, @Line, @ProductId,@Quantity, @UnitPrice, @SubTotal, @SD, @SDAmount, @VATRate, @VATAmount,@LineTotal, @UOMId,@UOMFromId, @UOMConversion, @IsPost, @Comments,@FixedVATAmount,@IsFixedVAT,@VATType,@OthersAmount,@TransactionType,@ReturnReason,@CtnQuantity,@PcsQuantity);
        SELECT SCOPE_IDENTITY();";
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn, transaction))
                {
                    cmd.Parameters.AddWithValue("@PurchaseReturnId", details.PurchaseReturnId);
                    cmd.Parameters.AddWithValue("@PurchaseId", details.PurchaseId ?? 0);
                    cmd.Parameters.AddWithValue("@PurchaseDetailId", details.PurchaseDetailId ?? 0);
        ...
        public async Task<ResultVM> Update(PurchaseReturnVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                SET 
                    ...
                    PurchaseReturnDate = @PurchaseReturnDate,  GrandTotalAmount = @GrandTotalAmount, 
                    GrandTotalSDAmount = @GrandTotalSDAmount, GrandTotalVATAmount = @GrandTotalVATAmount, 
                    Comments = @Comments,CurrencyId = @CurrencyId, CurrencyRateFromBDT = @CurrencyRateFromBDT, 
                    ImportIDExcel = @ImportIDExcel, FileName = @FileName,
...
SELECT 
    ...
    ISNULL(FORMAT(M.PurchaseReturnDate, 'yyyy-MM-dd'), '1900-01-01') AS PurchaseReturnDate,
...
WHERE 1 = 1 ";
    ...
    FROM (
        ...
        ROW_NUMBER() OVER(ORDER BY " + (options.sort.Count > 0 ? options.sort[0].field + " " + options.sort[0].dir : "H.Id DESC") + $@") AS rowindex,
                    ...
                    ISNULL(FORMAT(H.PurchaseReturnDate, 'yyyy-MM-dd'), '1900-01-01') AS PurchaseReturnDate,
                    ...
                    ISNULL(H.ImportIDExcel, '') AS ImportIDExcel,
...
";
        ...
        public async Task<ResultVM> GetDetailsGridData(GridOptions options, string[] conditionalFields, string[] conditionalValues, SqlConnection conn, SqlTransaction transaction)
        {
            ...
            try
            {
                ...
                var data = new GridEntity<PurchaseReturnDetail>();
    ...
    FROM PurchaseReturns H 
        LEFT OUTER JOIN BranchProfiles Br ON H.BranchId = Br.Id
        LEFT OUTER JOIN Suppliers S ON H.SupplierId = S.Id
        LEFT OUTER JOIN PurchaseReturnDetails D ON H.Id = D.PurchaseReturnId            
        LEFT OUTER JOIN Products P ON D.ProductId = P.Id
        LEFT OUTER JOIN Currencies c on H.CurrencyId = c.Id
        LEFT OUTER JOIN UOMs Us ON D.UOMId = Us.Id
	WHERE 1 = 1
    -- Add the filter condition
        " + (options.filter.Filters.Count > 0 ? " AND (" + GridQueryBuilder<PurchaseReturnDetail>.FilterCondition(options.filter) + ")" : "");
...
";
                ...
                data = KendoGrid<PurchaseReturnDetail>.GetTransactionalGridData_CMD(options, sqlQuery, "H.Id", conditionalFields, conditionalValues);
...
SELECT 
    ...
    ISNULL(FORMAT(M.PurchaseReturnDate, 'yyyy-MM-dd'), '1900-01-01') AS InvoiceDateTime,
    ISNULL(M.GrandTotalAmount,0) AS GrandTotalAmount,
    ISNULL(M.GrandTotalSDAmount, 0) AS GrandTotalSDAmount,
    ISNULL(M.GrandTotalVATAmount, 0) AS GrandTotalVATAmount,
