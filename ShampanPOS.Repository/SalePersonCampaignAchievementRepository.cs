...
namespace ShampanPOS.Repository
{
    ...
    public class SalePersonCampaignAchievementRepository : CommonRepository
    {
        // Insert Method
       
        public async Task<ResultVM> Insert(SalePersonCampaignAchievementVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                string query = @"
                    INSERT INTO SalePersonCampaignAchivements 
                    (
                    BranchId, SalePersonId, CampaignId,CampaignTargetId, TotalTarget, TotalSale, SelfSaleCommissionRate, 
                    OtherSaleCommissionRate, StartDate, EndDate,BonusAmount,OtherCommissionBonus,TotalBonus,IsPost, CreatedBy,CreatedFrom, CreatedOn
                    )
                    VALUES 
                    (
                    @BranchId, @SalePersonId, @CampaignId,@CampaignTargetId, @TotalTarget, @TotalSale, @SelfSaleCommissionRate, 
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn, transaction))
                {
                    ...
                    cmd.Parameters.AddWithValue("@CampaignTargetId", vm.CampaignTargetId);
        ...
        public async Task<ResultVM> Update(SalePersonCampaignAchievementVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            bool isNewConnection = false;
            ...
            try
            {
                ...
                string query = @"
                UPDATE SalePersonCampaignAchivements 
                SET 
                 SalePersonId=@SalePersonId, CampaignId=@CampaignId, TotalTarget=@TotalTarget, CampaignTargetId=@CampaignTargetId,
        ...
        public async Task<ResultVM> Delete(CommonVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                string query = $" UPDATE SalePersonCampaignAchivements SET IsArchive = 1, IsActive = 0,LastModifiedBy = @LastModifiedBy,LastUpdateFrom = @LastUpdateFrom ,LastModifiedOn =GETDATE() WHERE Id IN ({inClause})";
...
SELECT
    ...
    ISNULL(M.CampaignTargetId, 0) AS CampaignTargetId,
...
FROM SalePersonCampaignAchivements M
WHERE 1 = 1
";
                ...
                var modelList = dataTable.AsEnumerable().Select(row => new SalePersonCampaignAchievementVM
                {
                    Id = row.Field<int>("Id"),
                    BranchId = row.Field<int>("BranchId"),
                    SalePersonId = row.Field<int>("SalePersonId"),
                    CampaignId = row.Field<int>("CampaignId"),
                    CampaignTargetId = row.Field<int>("CampaignTargetId"),
...
FROM SalePersonCampaignAchivements
WHERE IsActive = 1
ORDER BY Id ";
        ...
        public async Task<ResultVM> MultiplePost(CommonVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                string query = $" UPDATE SalePersonCampaignAchivements SET IsPost = 1, PostedBy = @PostedBy , LastUpdateFrom = @LastUpdateFrom ,PostedOn = GETDATE() WHERE Id IN ({inClause}) ";               
        ...
        public async Task<ResultVM> GetGridData(GridOptions options, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                var data = new GridEntity<SalePersonCampaignAchievementVM>();

                // Define your SQL query string
                string sqlQuery = @"
    -- Count query
    SELECT COUNT(DISTINCT H.Id) AS totalcount
    FROM SalePersonCampaignAchivements  H
    ...
    LEFT OUTER JOIN SalePersonCampaignTargets camT ON H.CampaignTargetId = camT.Id

	WHERE 1 = 1
    -- Add the filter condition
    " + (options.filter.Filters.Count > 0 ? " AND (" + GridQueryBuilder<SalePersonCampaignAchievementVM>.FilterCondition(options.filter) + ")" : "") + @"
    ...
    FROM (
        ...
        ROW_NUMBER() OVER(ORDER BY " + (options.sort.Count > 0 ? options.sort[0].field + " " + options.sort[0].dir : "H.Id DESC") + @") AS rowindex ,
            ...
            ISNULL(camT.TotalTarget,0) CampaignTargetValue
...
";
                ...
                data = KendoGrid<SalePersonCampaignAchievementVM>.GetGridData_CMD(options, sqlQuery, "H.Id");
        ...
        public async Task<ResultVM> ProcessDataList(SalePersonCampaignAchievementDataVm vm, SqlConnection conn, SqlTransaction transaction)
        {
            bool isNewConnection = false;
            DataTable dataTable = new DataTable();
            ResultVM result = new ResultVM { Status = "Fail", Message = "Error", ExMessage = null, DataVM = null };
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SP_SalePersonCampaignAchivementProcess", conn, transaction))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    // Add parameters to the stored procedure
                    cmd.Parameters.AddWithValue("@SalePersonId", vm.SalePersonId);
                    cmd.Parameters.AddWithValue("@Date", vm.StartDate); 
                ...
                var modelList = dataTable.AsEnumerable().Select(row => new SalePersonCampaignAchievementVM
                {
                    ...
                    SalePersonId = row.Field<int>("SalePersonId"),
                    CampaignId = row.Field<int>("CampaignId"),
                    SalePersonName = row.Field<string>("Name"),
                    TotalTarget = row.Field<decimal>("TotalTarget"),
                    TotalSale = row.Field<decimal>("TotalSale"),
