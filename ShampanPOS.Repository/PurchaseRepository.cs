...
namespace ShampanPOS.Repository
{
    ...
    public class PurchaseRepository : CommonRepository
    {
            ...
            try
            {
                ...
                string query = @"
                    ...
                    (
                        ...
                        ImportIDExcel, FileName, CustomHouse, FiscalYear, PeriodId, CreatedBy, CreatedOn,CreatedFrom
                    )
                    VALUES 
                    (
                        @Code, @BranchId, @SupplierId, @BENumber, @InvoiceDateTime, @PurchaseDate, 
        ...
        (PurchaseId, PurchaseOrderId, PurchaseOrderDetailId, BranchId, Line, ProductId,Quantity, UnitPrice, SubTotal, SD, SDAmount, VATRate, VATAmount,LineTotal,UOMId,UOMFromId,UOMConversion,IsPost,Comments,FixedVATAmount,IsFixedVAT,VATType,OthersAmount,TransactionType,CtnQuantity,PcsQuantity)
        VALUES 
        (@PurchaseId, @PurchaseOrderId, @PurchaseOrderDetailId, @BranchId, @Line, @ProductId,@Quantity, @UnitPrice, @SubTotal, @SD, @SDAmount, @VATRate, @VATAmount,@LineTotal, @UOMId,@UOMFromId, @UOMConversion, @IsPost, @Comments,@FixedVATAmount,@IsFixedVAT,@VATType,@OthersAmount,@TransactionType,@CtnQuantity,@PcsQuantity);
        SELECT SCOPE_IDENTITY();";
        ...
        public async Task<ResultVM> Update(PurchaseVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                SET 
                    ...
                    InvoiceDateTime = @InvoiceDateTime, PurchaseDate = @PurchaseDate, GrandTotalAmount = @GrandTotalAmount, 
                    ...
                    ImportIDExcel = @ImportIDExcel, FileName = @FileName, CustomHouse = @CustomHouse, 
...
WHERE 1 = 1 ";
    ...
    FROM (
        ...
        ROW_NUMBER() OVER(ORDER BY " + (options.sort.Count > 0 ? options.sort[0].field + " " + options.sort[0].dir : "H.Id DESC") + $@") AS rowindex,
                ...
                CASE WHEN ISNULL(H.IsCompleted, 0) = 1 THEN 'Completed' ELSE 'Not-Completed' END AS Completed,
                ...
                ISNULL(H.CustomHouse, '') AS CustomHouse,
...
";
        ...
        public async Task<ResultVM> GetDetailsGridData(GridOptions options, string[] conditionalFields, string[] conditionalValues, SqlConnection conn, SqlTransaction transaction)
        {
            ...
            try
            {
                ...
                var data = new GridEntity<PurchaseDetail>();
	...
	WHERE 1 = 1
    -- Add the filter condition
        " + (options.filter.Filters.Count > 0 ? " AND (" + GridQueryBuilder<PurchaseDetail>.FilterCondition(options.filter) + ")" : "");
...
";
                ...
                data = KendoGrid<PurchaseDetail>.GetTransactionalGridData_CMD(options, sqlQuery, "H.Id", conditionalFields, conditionalValues);
	...
	LEFT JOIN 
					(
						SELECT d.PurchaseId, SUM(d.Quantity) AS TotalQuantity, SUM(d.CompletedQty) AS TotalCompletedQty
    ...
    FROM (
        ...
        LEFT JOIN 
					(
						SELECT d.PurchaseId, SUM(ISNULL(d.Quantity,0)) AS TotalQuantity, SUM(ISNULL(d.CompletedQty,0)) AS TotalCompletedQty
						FROM [dbo].[PurchaseDetails] d   
						GROUP BY d.PurchaseId
					) SD ON H.Id = SD.PurchaseId
        WHERE H.IsPost = 1 AND SD.TotalCompletedQty < SD.TotalQuantity
