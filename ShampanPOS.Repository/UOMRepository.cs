...
namespace ShampanPOS.Repository
{
    ...
    public class UOMRepository : CommonRepository
    {

        //Insert Method
        public async Task<ResultVM> Insert(UOMVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
...
SELECT SCOPE_IDENTITY();";
        ...
        //Update Method
        public async Task<ResultVM> Update(UOMVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            bool isNewConnection = false;
...
WHERE Id = @Id";
        ...
        public async Task<ResultVM> Delete(CommonVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                string query = $"UPDATE UOMs SET IsArchive = 1, IsActive = 0,LastModifiedBy = @LastModifiedBy,LastUpdateFrom = @LastUpdateFrom ,LastModifiedOn =GETDATE() WHERE Id IN ({inClause})";
...
ORDER BY Name";
    ...
    ISNULL(H.Code, '') AS Code,
    ISNULL(H.Name, '') AS Name,
	ISNULL(C.FromId, 0) AS FromId,
	ISNULL(Uf.Name,'') FromName,
	ISNULL(C.ToId, 0) AS ToId,
	ISNULL(UT.Name,'') ToName,
	ISNULL(C.ConversationFactor, 0) AS ConversationFactor,
    ISNULL(H.IsArchive, 0) AS IsArchive,
    ISNULL(H.IsActive, 0) AS IsActive,
    ISNULL(H.CreatedBy, '') AS CreatedBy,
    ISNULL(FORMAT(H.CreatedOn, 'yyyy-MM-dd HH:mm'), '1900-01-01') AS CreatedOn,
...
FROM 
    UOMs H
	LEFT OUTER JOIN UOMConversations C on H.Id = C.FromId
	 left outer join UOMs Uf on Uf.Id=C.FromId
	 left outer join UOMs Ut on Ut.Id=C.ToId
WHERE 
    1 = 1";
