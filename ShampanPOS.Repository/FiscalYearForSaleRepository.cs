...
namespace ShampanPOS.Repository
{
    ...
    public class FiscalYearForSaleRepository : CommonRepository
    {
        // Insert Method
        //        public async Task<ResultVM> Insert(FiscalYearForSaleVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        //        {
        ...
        //                string query = @"
        //INSERT INTO FiscalYearForSales 
        //(
        ...
        public async Task<ResultVM> Insert(FiscalYearForSaleVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn, transaction))
                {
                    ...
                    query = @" INSERT INTO FiscalYearDetailForSales (
                            FiscalYearForSaleId,
                            Year,
                            Remarks,
                            MonthId,
                    ...
                    foreach (FiscalYearDetailForSaleVM item in vm.fiscalYearForSaleDetails)
                    {
                        SqlCommand cmdDetails = new SqlCommand(query, conn, transaction);
                        cmdDetails.Parameters.AddWithValue("@FiscalYearForSaleId", vm.Id);
                        ...
                        cmdDetails.Parameters.AddWithValue("@CreatedOn", DateTime.Now);
        ...
        // Update Method
        //        public async Task<ResultVM> Update(FiscalYearForSaleVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        //        {
        //            bool isNewConnection = false;
        ...
        //                    query = @"UPDATE FiscalYearDetailForSales
        //                    SET
        //                        Year = @Year,
        //                        Remarks = @Remarks,
        //                        MonthId = @MonthId,
        ...
        //                        cmdDetails.Parameters.AddWithValue("@FiscalYearForSaleId", vm.Id);
        //                        cmdDetails.Parameters.AddWithValue("@Year", vm.Year);
        //                        cmdDetails.Parameters.AddWithValue("@Remarks", vm.Remarks ?? "-");
        //                        cmdDetails.Parameters.AddWithValue("@MonthId", item.MonthId ?? 0);
        //                        cmdDetails.Parameters.AddWithValue("@MonthStart", item.MonthStart);
        //                        cmdDetails.Parameters.AddWithValue("@MonthEnd", item.MonthEnd);
        //                        cmdDetails.Parameters.AddWithValue("@MonthName", item.MonthName);
        ...
        WHERE Id = @Id";
                ...
                // Update FiscalYearDetailForSales
                query = @"
        UPDATE FiscalYearDetailForSales
        SET
            Year = @Year,
            ...
            MonthLock = @MonthLock
        WHERE
            FiscalYearForSaleId = @FiscalYearForSaleId AND
            MonthId = @MonthId"; // Ensure MonthId is included in the WHERE clause

                foreach (FiscalYearDetailForSaleVM item in vm.fiscalYearForSaleDetails)
                {
                    using (SqlCommand cmdDetails = new SqlCommand(query, conn, transaction))
                    {
                        cmdDetails.Parameters.AddWithValue("@FiscalYearForSaleId", vm.Id);
                        cmdDetails.Parameters.AddWithValue("@Year", vm.Year);
                        cmdDetails.Parameters.AddWithValue("@Remarks", item.Remarks ?? "-");
        ...
        public async Task<ResultVM> Delete(CommonVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                string deleteFiscalYearsQuery = $"DELETE FROM FiscalYearForSales WHERE Id IN ({inClause})";

                // Delete FiscalYearDetails query
                string deleteFiscalYearDetailsQuery = $"DELETE FROM FiscalYearDetailForSales WHERE FiscalYearForSaleId IN ({inClause})";
                ...
                using (SqlCommand cmd = new SqlCommand(deleteFiscalYearsQuery, conn, transaction))
                {
                    ...
                    if (rowsAffected == 0)
                    {
                        throw new Exception("No rows were deleted from FiscalYears Sale.");
                    }
                }
        ...
        //        string query = $" Delete From FiscalYearForSales WHERE Id IN ({inClause})";
        ...
        //            {
        //                result.Status = "Success";
        //                result.Message = $"FiscalYearForSales with Ids {string.Join(", ", string.Join(",", vm.IDs))} deleted successfully.";
        //            }
        //            else
        //            {
 ...
 WHERE 1=1";
                ...
                var model = new List<FiscalYearForSaleVM>();

                foreach (DataRow row in dataTable.Rows)
                {
                    model.Add(new FiscalYearForSaleVM
                    {
                        Id = row.Field<int>("Id"),
                ...
                var detailsDataList = DetailsList(new[] { "D.FiscalYearForSaleId" }, conditionalValues, vm, conn, transaction);

                if (detailsDataList.Status == "Success" && detailsDataList.DataVM is DataTable dt)
                {
                    string json = JsonConvert.SerializeObject(dt);
                    var details = JsonConvert.DeserializeObject<List<FiscalYearDetailForSaleVM>>(json);

                    model.FirstOrDefault().fiscalYearForSaleDetails = details;
                }
...
ORDER BY Name";
        ...
        public async Task<ResultVM> GetGridData(GridOptions options, SqlConnection conn, SqlTransaction transaction)
        {
            ...
            try
            {
                ...
                var data = new GridEntity<FiscalYearForSaleVM>();
    ...
    " + (options.filter.Filters.Count > 0 ? " AND (" + GridQueryBuilder<FiscalYearForSaleVM>.FilterCondition(options.filter) + ")" : "") + @"
...
";
                ...
                data = KendoGrid<FiscalYearForSaleVM>.GetGridData_CMD(options, sqlQuery, "H.Id");
        ...
        public ResultVM DetailsList(string[] conditionalFields, string[] conditionalValue, PeramModel vm = null, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                ,[FiscalYearForSaleId]
                      ...
                      ,[Remarks]
                      ,[MonthId]
                FROM FiscalYearDetailForSales D
                where 1=1";
        ...
        public bool DuplicateFiscal(int? year, SqlConnection conn, SqlTransaction transaction)
        {
            bool isNewConnection = false;
            bool exists = false;
