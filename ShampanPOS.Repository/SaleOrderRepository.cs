    ...
    public class SaleOrderRepository : CommonRepository
    {
        ...
        public async Task<ResultVM> Insert(SaleOrderVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                (
                    Code, BranchId, CustomerId, SalePersonId, RouteId, DeliveryAddress, InvoiceDateTime, 
                    DeliveryDate, GrandTotalAmount, GrandTotalSDAmount, GrandTotalVATAmount,RegularDiscountRate,RegularDiscountAmount,SpecialDiscountRate,SpecialDiscountAmount, Comments, 
                    TransactionType, IsCompleted, CurrencyId, CurrencyRateFromBDT, Latitude,Longitude,CreatedBy, CreatedOn,InvoiceDiscountRate,InvoiceDiscountAmount
                )
                VALUES 
                (
                    @Code, @BranchId, @CustomerId, @SalePersonId, @RouteId, @DeliveryAddress, @InvoiceDateTime, 
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn, transaction))
                {
                    ...
                    cmd.Parameters.AddWithValue("@RegularDiscountRate", vm.RegularDiscountRate ?? 0);
                    cmd.Parameters.AddWithValue("@RegularDiscountAmount", vm.RegularDiscountAmount ?? 0);
                    cmd.Parameters.AddWithValue("@SpecialDiscountRate", vm.SpecialDiscountRate ?? 0);
                    cmd.Parameters.AddWithValue("@SpecialDiscountAmount", vm.SpecialDiscountAmount ?? 0);
                    ...
                    cmd.Parameters.AddWithValue("@IsCompleted", vm.IsCompleted ?? false );
        ...
        public async Task<ResultVM> Update(SaleOrderVM vm, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                SET 
                    ...
                    RouteId = @RouteId, DeliveryAddress = @DeliveryAddress, InvoiceDateTime = @InvoiceDateTime, 
                    DeliveryDate = @DeliveryDate, GrandTotalAmount = @GrandTotalAmount, GrandTotalSDAmount = @GrandTotalSDAmount, 
                    GrandTotalVATAmount = @GrandTotalVATAmount, Comments = @Comments, TransactionType = @TransactionType, 
                     CurrencyId = @CurrencyId, CurrencyRateFromBDT = @CurrencyRateFromBDT, 
                    ...
                    , RegularDiscountRate=@RegularDiscountRate
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn, transaction))
                {
                    ...
                    cmd.Parameters.AddWithValue("@InvoiceDateTime", vm.InvoiceDateTime);
                    cmd.Parameters.AddWithValue("@DeliveryDate", vm.DeliveryDate);
                    cmd.Parameters.AddWithValue("@GrandTotalAmount", vm.GrandTotalAmount);
                    cmd.Parameters.AddWithValue("@GrandTotalSDAmount", vm.GrandTotalSDAmount ?? 0);
                    cmd.Parameters.AddWithValue("@GrandTotalVATAmount", vm.GrandTotalVATAmount ?? 0);
                    cmd.Parameters.AddWithValue("@Comments", vm.Comments ?? (object)DBNull.Value);
                    cmd.Parameters.AddWithValue("@TransactionType", vm.TransactionType ?? (object)DBNull.Value);
                    cmd.Parameters.AddWithValue("@CurrencyId", vm.CurrencyId ?? (object)DBNull.Value);
                    cmd.Parameters.AddWithValue("@CurrencyRateFromBDT", vm.CurrencyRateFromBDT ?? (object)DBNull.Value);
            ...
            try
            {
                ...
                string query = @"
                            SELECT
                                ...
                                ISNULL(CR.Name, '') AS CurrencyName,
	                            ...
	                            ISNULL(M.InvoiceDiscountAmount,0) AS InvoiceDiscountAmount,
                                  ISNULL(M.Latitude, 0) AS Latitude,
                                ISNULL(M.Longitude, 0) AS Longitude
                            FROM SaleOrders M
							LEFT OUTER JOIN Routes R on M.RouteId = R.Id
							LEFT OUTER JOIN Customers C on M.CustomerId = C.Id
							LEFT OUTER JOIN SalesPersons SP on M.SalePersonId = SP.Id
                            LEFT OUTER JOIN Currencies CR on M.CurrencyId = CR.Id
        ...
        public async Task<ResultVM> InsertDetails(SaleOrderDetailVM details, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                string query = @"
                    ...
                    (SaleOrderId, BranchId, Line, ProductId,CtnQuantity,PcsQuantity, Quantity, UnitRate, SubTotal, IsInclusiveDuty, SD, SDAmount, VATRate, VATAmount, LineTotal, UOMId, UOMFromId, UOMConversion,CustomerId, FreeProductId, FreeQuantity, DiscountRate, DiscountAmount, SubTotalAfterDiscount, LineDiscountRate, LineDiscountAmount, LineTotalAfterDiscount, Comments,CampaignId,CampaignDetailsId,CampaignTypeId)
                    VALUES 
                    (@SaleOrderId, @BranchId, @Line, @ProductId,@CtnQuantity,@PcsQuantity, @Quantity, @UnitRate, @SubTotal, @IsInclusiveDuty, @SD, @SDAmount, @VATRate, @VATAmount, @LineTotal, @UOMId, @UOMFromId, @UOMConversion,@CustomerId, @FreeProductId, @FreeQuantity, @DiscountRate, @DiscountAmount, @SubTotalAfterDiscount, @LineDiscountRate, @LineDiscountAmount, @LineTotalAfterDiscount, @Comments,@CampaignId,@CampaignDetailsId,@CampaignTypeId);
                    SELECT SCOPE_IDENTITY();";
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn, transaction))
                {
                    cmd.Parameters.AddWithValue("@SaleOrderId", details.SaleOrderId ?? (object)DBNull.Value);
                    ...
                    cmd.Parameters.AddWithValue("@IsInclusiveDuty", details.IsInclusiveDuty ?? false);
                    ...
                    cmd.Parameters.AddWithValue("@UOMConversion", details.UOMConversion ?? (object)DBNull.Value);
        ...
        public ResultVM DetailsList(string[] conditionalFields, string[] conditionalValue, PeramModel vm = null, SqlConnection conn = null, SqlTransaction transaction = null)
        {
            ...
            try
            {
                ...
                ISNULL(D.IsInclusiveDuty,0) as IsInclusiveDuty,
...
";
        ...
        public async Task<ResultVM> GetDetailsGridData(GridOptions options, string[] conditionalFields, string[] conditionalValues, SqlConnection conn, SqlTransaction transaction)
        {
            ...
            try
            {
                ...
                var data = new GridEntity<SaleOrderDetails>();
...
LEFT OUTER JOIN BranchProfiles BF ON H.BranchId = BF.Id
    ...
    LEFT OUTER JOIN Products PF ON D.ProductId = PF.Id
    LEFT OUTER JOIN UOMs U ON D.UOMId = U.Id
    LEFT OUTER JOIN Campaigns CP ON D.CampaignId = CP.Id

WHERE 1 = 1
-- Add the filter condition
" + (options.filter.Filters.Count > 0 ? " AND (" + GridQueryBuilder<SaleOrderDetails>.FilterCondition(options.filter) + ")" : "");
...
FROM (
    ...
    ISNULL(PF.Name, '') AS FreeProductName,
    ISNULL(D.CampaignId, 0) AS CampaignId,
    ISNULL(D.FreeProductId, 0) AS FreeProductId,
    ...
    LEFT OUTER JOIN SaleOrderDetails D ON H.Id = D.SaleOrderId
    LEFT OUTER JOIN Products PD ON D.ProductId = PD.Id
    LEFT OUTER JOIN Products PF ON D.FreeProductId = PF.Id
    LEFT OUTER JOIN UOMs U ON D.UOMId = U.Id
    LEFT OUTER JOIN Campaigns CP ON D.CampaignId = CP.Id
...
WHERE rowindex > @skip AND (@take = 0 OR rowindex <= @take);";
                ...
                data = KendoGrid<SaleOrderDetails>.GetTransactionalGridData_CMD(options, sqlQuery, "H.Id", conditionalFields, conditionalValues);
